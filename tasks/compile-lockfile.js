...
try {
  ...
  // Extract the dependencies from occ-react-scripts (which is a workspace)
  const dependencies = require('occ-react-scripts/package.json').dependencies;
  const descriptors = Object.keys(dependencies).map(
    dep => `${dep}@${dependencies[dep]}`
  );

  // Run "yarn add" with all the dependencies of occ-react-scripts
  cprocess.execFileSync('yarn', ['add', ...descriptors], { cwd: temp });

  // Store the generated lockfile in create-react-app
  // We can't store it inside occ-react-scripts, because we need it even before occ-react-scripts is installed
  fse.copySync(
